<template>
  <div>
    <div class="form-group">
      ...
      <div class="col-sm-12">Định dạng banner</div>
      <div class="col-sm-12">
        <div class="table-responsive">
          <table class="table table-striped table-bordered table-hover">
            ...
            <tbody v-if="_getBannerFormatList.length">
              <item-format-banner
                v-for="(item, idx) in _getBannerFormatList"
                :key="idx"
                :banner="item"
              ></item-format-banner>
            </tbody>
            ...
            <thead>
              <tr>
                <td style="width: 10%" class="text-left">
                  {{ $options.setting.image_title }}
                </td>
                <td style="width: 35%" class="text-left">Tiêu đề</td>
                <td style="width: 25%">
                  {{ $options.setting.image_url_title }}
                </td>
                <td>Width</td>
                <td>Height</td>
                <td>Trạng thái</td>
...
export default {
  ...
  methods: {
    ...mapActions(MODULE_MODULE_NOI_BAT, ['updateSettingByKey']),
    _changeImage(fi) {
      if (typeof fi === 'object') {
