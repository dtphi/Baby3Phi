    ...
    <div :class="classShow" :style="styleCss">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
              <template v-if="_errors">
                <div class="alert alert-danger">
                  ...
                  <p v-for="(err, idx) in _errorToArrs()" :key="idx">
                    {{ err }}
                  </p>
                </div>
              </template>
                ...
                <form class="form-horizontal">
                  ...
                  <div class="form-group required">
                    <label
                      ...
                      class="col-sm-2 control-label"
                      >{{ $options.setting.passwordTxt }}</label
                    >
                    <div class="col-sm-10">
                      <validation-provider
                        :immediate="false"
          ...
          <template v-if="user">
            <validation-observer
              ref="observerUserPermission"
              @submit.prevent="_submitUserPermission"
              v-if="_isRules"
            >
              <div class="modal-body">
                <form class="form-horizontal">
                  <div
                    class="form-group"
                    v-for="(item, idx) in user.ruleSelect"
                    :key="idx"
                  >
                    <label
                      for="input-user-password-"
                      class="col-sm-3 control-label"
                      >{{ $options.setting.permisstionGroupTexts[idx] }}</label
                    >
                    <div class="col-sm-9">
                      <input
                    ...
                    <div class="row">
                      <div
                        class="col-sm-2"
                        v-for="(rule, id) in item.abilities"
                        :key="id"
                      >
                        <label
                          for="input-user-password-"
                          class="control-label"
                          >{{
                            $options.setting.permissionActionTexts[id]
                          }}</label
                        >
                        <input
              ...
              <div class="modal-footer">
                ...
                <button
                  type="button"
                  class="btn btn-success"
                  @click="_submitUserPermission"
                >
                  {{ $options.setting.btnSubmitRuleTxt }}
                </button>
              </div>
            </validation-observer>
          </template>
...
import { MODULE_USER_EDIT_MODAL, } from 'store@admin/types/module-types'
import {
  ACTION_CLOSE_MODAL,
  ACTION_SET_LOADING,
  ACTION_UPDATE_USER,
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(MODULE_USER_EDIT_MODAL, ['classShow', 'styleCss', 'user']),
    _errors() {
      return this.errors.length
    },
    _isRules() {
      if (fnCheckProp(this.user, 'ruleSelect')) {
  ...
  setting: {
    ...
    permisstionGroupTexts: {
      ...
      slide_info_specials: 'Tin tức/ Slide tiêu điểm',
      album_group: 'Album hình ảnh/ Danh mục',
      album: 'Album hình ảnh/ Album',
    },
