...
use App\Contacts\Apis\Admins\Repositories\Information as InfoRepo;
...
final class InformationRepository implements BaseRepo, InfoRepo
{
    /**
    ...
    public function apiInsert($data = [])
    {
        ...
        if ($this->model->save()) {
            ...
            InformationDescription::insertByInfoId(
                $infoId,
                $data['name'],
            ...
            if (isset($data['info_images']) && !empty($data['info_images'])) {
                foreach ($data['info_images'] as $information_image) {
                    InformationImage::insertByInfoId(
                        $infoId,
    ...
    public function apiGetBuilderInformations($data = [], $limit = 5)
    {
        ...
        $query = DB::table(Tables::tbl_informations)->leftJoin(
            Tables::tbl_information_descriptions . ' AS ' . $alias,
    ...
    public function importInformation()
    {
        ...
        foreach ($results as $info) {
            ...
            ModelRelationsInformationDescription::insertByInfoId(
                $infoId,
                $info->newsgroupname,
                $des,
                $info->tag,
