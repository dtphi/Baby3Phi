...
use App\Services\Apis\Fronts\InformationPages\BasicInformation;
...
class NewsController extends Controller
{
     ...
     */
    public function __construct(private BasicInformation $bsSv, protected BasicSetting $settingSv, array $middleware = [])
    {
    ...
    {
        return $this->bsSv->informationRepository();
    }
    ...
    public function list(Request $request)
    {
        ...
        if (isset($params['category_id']) && in_array($params['category_id'], self::$menuFullInfos)) {
            $subCategory = $this->bsSv->informationRepository()->apiGetMenuCategoryIds($params['category_id']);
        ...
        $results    = $this->bsSv->informationRepository()->apiGetInfoList($params);
        ...
        $staticImg     = NO_FRONT_THUMB_IMG;
        $staticThumImg = NO_FRONT_THUMB_IMG;
        $infos         = [];
    ...
    public function detail(Request $request)
    {
        ...
        if (isset($params['information_id'])) {
            $this->bsSv->informationRepository()->apiUpdateViewed($params['information_id']);

            $json['results']                  = $this->bsSv->informationRepository()->apiGetInfo($params['information_id']);
    ...
    public function showLastedList(Request $request)
    {
        ...
        $list = $this->bsSv->informationRepository()->apiGetLatestInfos(20)->toArray();
        ...
        if (!empty($list)) {
            ...
            if (!empty($infoIds)) {
                ...
                $results = $this->bsSv->informationRepository()->apiGetInfoListByIds($params);
    ...
    public function showPopularList(Request $request)
    {
        ...
        $list = $this->bsSv->informationRepository()->apiGetPopularInfos(20)->toArray();
    ...
    public function showRelatedList($informationId = null, Request $request)
    {
        ...
        $list = $this->bsSv->informationRepository()->apiGetInfoRelated($informationId);

        if ($list) {
            foreach ($list as $info) {
                $json[$info->related_id] = $this->bsSv->informationRepository()->apiGetInfo($info->related_id);
            }
        }
    ...
    public function showSpecialModuleList(Request $request)
    {
        ...
        if (!empty($params['information_ids'])) {
            $infoCarousels = $this->bsSv->informationRepository()->apiGetInfoCarouselListByIds($params);
