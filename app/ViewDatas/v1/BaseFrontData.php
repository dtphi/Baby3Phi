...
class BaseFrontData
{
    ...
    public function __construct()
    {
        ...
        if (fn_is_prod_env()) {
            $this->settings['pageDir'] = config('app.is_mix') ? mix('js/b3p-front-' . config('app.api_name_key') . '.js'): asset('js/b3p-front-' . config('app.api_name_key') . '.js');
        } else if(fn_is_stg_env()) {
            $this->settings['pageDir'] = config('app.is_mix') ? mix('js/stg/b3p-app-front.js'): asset('js/stg/b3p-app-front.js');
        } else {
            $this->settings['pageDir'] = config('app.is_mix') ? mix('js/stg/b3p-app-front.js'): asset('js/b3p-app-front.js');
        }
    }
