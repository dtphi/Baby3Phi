...
use App\Contacts\Apis\Admins\Models\Admin as ModelAdmin;
...
class Admin extends Authenticatable implements ModelAdmin
{
    use HasApiTokens;
    use HasFactory;
    use Notifiable;
    ...
    {
        $accessToken = PersonalAccessToken::where('tokenable_id', Auth::user()->id)
            ->where('name', $name)
            ->first();
